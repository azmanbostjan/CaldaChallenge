...
CREATE POLICY "No updates or deletes" ON stg.stg_orders
FOR UPDATE, DELETE
USING (false)
WITH CHECK (false);

CREATE INDEX idx_stg_orders_user_id ON stg.stg_orders(user_id);
CREATE INDEX idx_stg_orders_archived_at ON stg.stg_orders(archived_at);
