...
CREATE POLICY "Public read access" ON items_catalog
FOR SELECT
USING (true);

-- Admins can modify items_catalog
CREATE POLICY "Admins full access" ON items_catalog
FOR ALL
USING (auth.role() = 'admin')
WITH CHECK (auth.role() = 'admin');
...
CREATE POLICY "Admins can view item history" ON item_history
FOR SELECT
USING (auth.role() = 'admin');

-- No manual update allowed
CREATE POLICY "No manual insert into item_history" ON item_history
FOR INSERT
WITH CHECK (false);

-- No manual update allowed
CREATE POLICY "No manual update of item_history" ON item_history
FOR UPDATE
USING (false)
WITH CHECK (false);

-- No manual delete allowed
CREATE POLICY "No manual delete from item_history" ON item_history
FOR DELETE
USING (false);
